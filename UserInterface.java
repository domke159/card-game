...
public class UserInterface {
  ...
  public void printWinnerOfGame(Player player){
    System.out.println("\nWoohoo!!! Winner of game is: " + player.getName());
  }
  ...
  /*If a wrong number is entered when user needs to choose it allows the user to try again,
    however, when a symbol other than a number is entered an InputMismatchException is being thrown and the user cannot try again. 
    The following implementation would allow the user to try again regardless of the input type.
  ...
  public ArrayList<Player> getPlayerInformation() {
    ...
    System.out.println("\nHow many ai players [" + min + "-" + (Constants.MAX_NO_OF_PLAYERS - noOfPlayers) + "]:"); // it would be better not to print this if there are 4 human players
    noOfPlayers = getIntInput(min, Constants.MAX_NO_OF_PLAYERS - noOfPlayers);
    // for each ai player, get name
    for (int i = 0; i < noOfPlayers; i++) {
      // players.add(new Player()); - commented out
      Player p = new Player();
      players.add(p);
