...
public class Switch {
  ...
  private boolean runPlayer(Player player) {
    ...
    } else { // better design would be to start the else statement in a new line
      boolean inplay = true;
      while (inplay) {
  ...
  private Card getCard() {
    ...
    if (stock.isEmpty()) {
      ...
      /* Handling the situation where the players are not discarding any cards and only picking them from stock
      * 
      System.out.println("There are no cards to pick from");
      return null; */
    }
  ...
  private boolean discardCard(Player player, Card card) {
    boolean discarded = false;
    if (isValidDiscard(card) && player.getHand().contains(card)) {
